...
def scatter_plot(x, y, plot_name, x_label, y_label, xyline, title):
        '''
...
def main():
        ...
        for f in filenames:
                ...
                if abs(train_stats['Max x']) > abs(train_stats['Consensus energy']):
                    count += 1
                ...
                #hmm_cse, hmm_modes, refined_cse = [], [], []
                if abs(train_stats['Consensus energy']) <= abs(test_stats['Consensus energy']):
                ...
                if abs(train_stats['Mode']) > abs(test_stats['Mode']):
        ...
        plt.title('Intersecting cases of positions of consensus and mode')
        ...
        plt.ylabel('Number of Sequences')
        plt.xticks(rotation = 90)
        plt.title('Distribution of positions of consensus and mode')
        ...
        scatter_plot(refined_cse, hmm_cse, 'refined_cs_vs_hmm_cs_lowhmmcsfamilies.png', 'Refined Alignment Consensus Energies', 'HMM Sequences Consensus Energies', True,
                'refined cs energy vs hmm cs energy (families where refined cs > hmm cs)')
        ...
        scatter_plot(mode_train_lowhmm, mode_test_lowhmm, 'refined_mode_vs_hmm_mode_lowhmmcsfamilies.png', 'Refined Alignment Mode Energies', 'HMM Sequences Mode Energies',True,
                'refined mode energy vs hmm mode energy (families where refined cs > hmm cs)')
        ...
        scatter_plot(mean_train_lowhmm, mean_test_lowhmm, 'refined_mean_vs_hmm_mean_lowhmmcsfamilies.png', 'Refined Alignment Mean Energies', 'HMM Sequences Mean Energies', True, 
                'refined mean energy vs hmm mean energy (families where refined cs > hmm cs)')
        ...
        plot_name = 'cool_plots/' + 'consensus_energy_vs_mode_energy.png'
        scatter_plot(refined_consensus_energies, refined_mode_energies, 'consensus_energy_vs_mode_energy.png',
                'Consensus Energy', 'Mode Energy', True, 'Consensus energy vs Mode energy for refined alignments')
        ...
        scatter_plot(refined_consensus_energies, refined_mean_energies, 'consensus_energy_vs_mean_energy.png',
               'Consensus Energy', 'Mean Energy', True, 'Consensus energy vs Mean energy for refined alignments')

        #refined mode energy vs hmm mode enegy
        scatter_plot(refined_mode_energies, hmm_mode_energies, 'refined_mode_vs_hmm_mode.png',
                'Mode Energy (Refined Alignment)', 'Mode Energy (HMM Alignment)', True, 'Refined mode energy vs HMM mode energy')
        ...
        ax.bar(labels, l[0: 2])
        plt.ylabel('Number of sequences')
        plt.title('These numbers include all processed families')
